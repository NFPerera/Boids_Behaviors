    ...
    public class BoidsController : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            m_model.Move((gameObject.transform.forward + l_wantedDir),
                m_model.GetData().GetStatById(BoidsStatsIds.MovementSpeed) * GetCurrSpeedBasedOnDistance(0.02f));
        }
        /*
        ...
        {
            var l_size = Physics.RaycastNonAlloc(transform.position, transform.forward, m_obstacleRayHit, 
                m_model.GetData().GetStatById(BoidsStatsIds.ViewRange), m_model.GetData().ObstacleMask);
        
            if (l_size < 1)
                return 1.0f; // Return 1 when there are no obstacles

            var l_closestDistanceToObs = m_model.GetData().GetStatById(BoidsStatsIds.ViewRange);
